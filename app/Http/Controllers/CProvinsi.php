    ...
    public function data()
    {
        ...
        return DataTables::eloquent($model)
            ->addColumn('action', function ($row) {
                $btn = '';                                
                $btn .= '<a href="'.url('provinsi/detail/'.$row->id_provinsi).'" class="text-warning mr-2"><span class="mdi mdi-information-outline"></span></a>';                
                $btn .= '<a href="'.url('provinsi/show/'.$row->id_provinsi).'" class="text-danger mr-2"><span class="mdi mdi-pen"></span></a>';                
                $btn .= '<a href="'.url('provinsi/delete/'.$row->id_provinsi).'" class="text-primary delete"><span class="mdi mdi-delete"></span></a>';
                return $btn;
            })
            ->rawColumns(['action'])
            ->addIndexColumn()
